            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                ...
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        ...
                        <form method="POST" action="{{ route('logout') }}">{{ csrf_field() }}<x-dropdown-link :href="route('logout')" onclick="event.preventDefault(); this.closest('form').submit();">{{ __('Log Out') }}</x-dropdown-link></form>
                    </x-slot>
                </x-dropdown>
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @auth
                ...
                <x-responsive-nav-link :href="route('kategori.index')" :active="request()->routeIs('kategori.*')">{{ __('Kategori') }}</x-responsive-nav-link>
                @if (Auth::user()->role == 'admin')
                    <x-responsive-nav-link :href="route('penerbit.index')" :active="request()->routeIs('penerbit.*')">{{ __('Penerbit') }}</x-responsive-nav-link>
                    <x-responsive-nav-link :href="route('peminjaman.index')" :active="request()->routeIs('peminjaman.*')">{{ __('Peminjaman') }}</x-responsive-nav-link>
                @endif
            @endauth
        </div>
        <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
            ...
            <div class="mt-3 space-y-1">
                ...
                <div x-data="themeSwitcher" class="border-t border-gray-200 dark:border-gray-600 pt-2 mt-2">
                    ...
                    <x-responsive-nav-link href="#" @click.prevent="applyTheme('light')">Light</x-responsive-nav-link>
                    <x-responsive-nav-link href="#" @click.prevent="applyTheme('dark')">Dark</x-responsive-nav-link>
                    <x-responsive-nav-link href="#" @click.prevent="applyTheme('system')">System</x-responsive-nav-link>
                </div>
                <form method="POST" action="{{ route('logout') }}">{{ csrf_field() }}<x-responsive-nav-link :href="route('logout')" onclick="event.preventDefault(); this.closest('form').submit();">{{ __('Log Out') }}</x-responsive-nav-link></form>
            </div>
        </div>
    </div>
</nav>