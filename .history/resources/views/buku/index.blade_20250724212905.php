                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-6 gap-y-8">
                        @forelse ($buku as $item)
                            <div class="relative flex flex-col bg-gray-50 dark:bg-gray-800/50 rounded-lg shadow p-4 transition-transform duration-300 hover:-translate-y-1">
                                {{-- Gambar Sampul --}}
                                <div class="aspect-h-3 aspect-w-2 w-full overflow-hidden rounded-md bg-gray-200 dark:bg-gray-700 mb-4">
                                    @if($item->sampul)
                                ...
                                <div class="flex flex-col flex-1">
                                    <h3 class="text-sm font-semibold text-gray-900 dark:text-white leading-tight truncate">{{ $item->judul_buku }}</h3>
                                    <p class="mt-1 text-sm text-gray-500 truncate">{{ $item->penerbit->nama_penerbit }}</p>
                                    ...
                                    <p class="mt-2 text-sm font-bold text-indigo-600 dark:text-indigo-400">Stok: {{ $item->stok }}</p>

                                    {{-- Tombol Aksi untuk Admin --}}
                                    @if (Auth::user()->role == 'admin')
                                    <div class="mt-auto pt-4 flex items-center gap-2 border-t border-gray-200 dark:border-gray-700 mt-4">
