<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    {{-- ======================================================= --}}
                    {{-- === TAMPILAN BARU: GRID KARTU KATEGORI (RESPONSIF) === --}}
                    {{-- ======================================================= --}}
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        @forelse ($kategori as $item)
                            {{-- Setiap kartu adalah link untuk memfilter buku --}}
                            <a href="{{ route('buku.index', ['kategori' => $item->nama_kategori]) }}"
                                class="group block bg-gray-50 dark:bg-gray-800/50 rounded-lg shadow p-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:ring-2 hover:ring-indigo-500">
                                <div class="flex flex-col justify-between h-full">
                                    <div>
                                        ...
                                        {{-- Menampilkan jumlah buku dalam kategori ini --}}
    ...
    @push('scripts')
        {{-- Script SweetAlert akan otomatis berfungsi di sini --}}
    @endpush
</x-app-layout>
