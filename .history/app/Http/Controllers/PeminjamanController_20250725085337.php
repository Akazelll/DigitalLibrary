...
class PeminjamanController extends Controller
{
    ...
    public function edit(string $id)
    {
        DB::transaction(function () use ($id) {
            ...
            $denda = 0;
            // Hitung denda HANYA jika terlambat saat dikembalikan
            if ($peminjaman->is_overdue) {
                ...
                // Pastikan hari terlambat lebih dari 0
                if ($hariTerlambat > 0) {
                    $denda = $hariTerlambat * Peminjaman::DENDA_PER_HARI;
                }
            }

            // Update status, tanggal kembali, dan denda
            $peminjaman->status = 'kembali';
            $peminjaman->tgl_kembali = now();
            $peminjaman->denda = $denda; // Simpan denda ke database
            $peminjaman->save();
