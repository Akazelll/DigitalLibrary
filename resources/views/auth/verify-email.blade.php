    ...
    <div class="min-h-screen flex flex-col sm:justify-center items-center pt-6 sm:pt-0 bg-base">
        <div class="w-full sm:max-w-md mt-6 px-6 py-8 bg-surface shadow-md overflow-hidden sm:rounded-lg">
            ...
            @if (session('status') == 'verification-link-sent')
                <div class="mb-4 font-medium text-sm text-green-800 p-3 rounded-md bg-green-50">
            ...
            <div class="mt-4 flex items-center justify-between">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                        class="underline text-sm text-text-subtle hover:text-text-main rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
                        {{ __('Log Out') }}
                    </button>
                </form>
            </div>
